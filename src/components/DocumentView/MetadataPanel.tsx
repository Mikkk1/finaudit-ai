...
const MetadataPanel = () => {
  const document = getDummyDocument('1');
  const [isEditing, setIsEditing] = useState(false);
  const [editedMetadata, setEditedMetadata] = useState({ ...document });
  ...
  const renderComplexValue = (key, value, depth = 0) => {
            ...
            <div className="ml-4 pl-4 border-l-2 border-light-border">
              {Object.entries(value).map(([nestedKey, nestedValue]) => (
                <div key={nestedKey} className="py-2">
                  ...
                  <div className="mt-1">
                    {renderValue(nestedKey, nestedValue, depth + 1)}
                  </div>
                </div>
              ))}
            </div>
            ...
            >
              <div className="flex flex-col gap-4">
                <div className="flex items-center">
                  <label
                    htmlFor={key}
                    className="text-sm font-medium text-slate-gray uppercase tracking-wide"
                  >
                    {key}
                  </label>
                </div>
